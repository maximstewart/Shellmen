...
:class:`~libs.prompt_toolkit.interface.CommandLineInterface` instance for you.
...
def create_eventloop(inputhook=None, recognize_win32_paste=True):
    ...
    :class:`~libs.prompt_toolkit.eventloop.base.EventLoop` instance for a
    :class:`~libs.prompt_toolkit.interface.CommandLineInterface`.
    """
    if is_windows():
        from libs.prompt_toolkit.eventloop.win32 import Win32EventLoop as Loop
        return Loop(inputhook=inputhook, recognize_paste=recognize_win32_paste)
    else:
        from libs.prompt_toolkit.eventloop.posix import PosixEventLoop as Loop
        return Loop(inputhook=inputhook)


def create_output(stdout=None, true_color=False, ansi_colors_only=None):
    """
    Return an :class:`~libs.prompt_toolkit.output.Output` instance for the command
    line.

    :param true_color: When True, use 24bit colors instead of 256 colors.
        (`bool` or :class:`~libs.prompt_toolkit.filters.SimpleFilter`.)
...
def create_asyncio_eventloop(loop=None):
    """
    Returns an asyncio :class:`~libs.prompt_toolkit.eventloop.EventLoop` instance
    for usage in a :class:`~libs.prompt_toolkit.interface.CommandLineInterface`. It
    ...
    if is_windows():
        from libs.prompt_toolkit.eventloop.asyncio_win32 import Win32AsyncioEventLoop as AsyncioEventLoop
    else:
        from libs.prompt_toolkit.eventloop.asyncio_posix import PosixAsyncioEventLoop as AsyncioEventLoop
...
def create_prompt_layout(message='', lexer=None, is_password=False,
    ...
    :param lexer: :class:`~libs.prompt_toolkit.layout.lexers.Lexer` to be used for
        the highlighting.
    :param is_password: `bool` or :class:`~libs.prompt_toolkit.filters.CLIFilter`.
    ...
    :param display_completions_in_columns: `bool` or
        :class:`~libs.prompt_toolkit.filters.CLIFilter`. Display the completions in
        multiple columns.
    :param multiline: `bool` or :class:`~libs.prompt_toolkit.filters.CLIFilter`.
        When True, prefer a layout that is more adapted for multiline input.
        Text after newlines is automatically indented, and search/arg input is
        shown below the input, instead of replacing the prompt.
    :param wrap_lines: `bool` or :class:`~libs.prompt_toolkit.filters.CLIFilter`.
...
def create_prompt_application(
    ...
    :param complete_while_typing: `bool` or
        :class:`~libs.prompt_toolkit.filters.SimpleFilter`. Enable autocompletion
        while typing.
    :param enable_history_search: `bool` or
        :class:`~libs.prompt_toolkit.filters.SimpleFilter`. Enable up-arrow parting
        string matching.
    :param lexer: :class:`~libs.prompt_toolkit.layout.lexers.Lexer` to be used for
        the syntax highlighting.
    :param validator: :class:`~libs.prompt_toolkit.validation.Validator` instance
        for input validation.
    :param completer: :class:`~libs.prompt_toolkit.completion.Completer` instance
        for input completion.
    :param reserve_space_for_menu: Space to be reserved for displaying the menu.
        (0 means that no space needs to be reserved.)
    :param auto_suggest: :class:`~libs.prompt_toolkit.auto_suggest.AutoSuggest`
        instance for input suggestions.
    :param style: :class:`.Style` instance for the color scheme.
    :param enable_system_bindings: `bool` or
        :class:`~libs.prompt_toolkit.filters.CLIFilter`. Pressing Meta+'!' will show
        a system prompt.
    :param enable_open_in_editor: `bool` or
        :class:`~libs.prompt_toolkit.filters.CLIFilter`. Pressing 'v' in Vi mode or
        C-X C-E in emacs mode will open an external editor.
    :param history: :class:`~libs.prompt_toolkit.history.History` instance.
    :param clipboard: :class:`~libs.prompt_toolkit.clipboard.base.Clipboard` instance.
        (e.g. :class:`~libs.prompt_toolkit.clipboard.in_memory.InMemoryClipboard`)
    :param get_bottom_toolbar_tokens: Optional callable which takes a
        :class:`~libs.prompt_toolkit.interface.CommandLineInterface` and returns a
    ...
    :param get_title: Callable that returns the title to be displayed in the
        terminal.
    :param mouse_support: `bool` or :class:`~libs.prompt_toolkit.filters.CLIFilter`
...
def prompt(message='', **kwargs):
    ...
    This is a wrapper around a lot of ``libs.prompt_toolkit`` functionality and can
    be a replacement for `raw_input`. (or GNU readline.)

    If you want to keep your history across several calls, create one
    :class:`~libs.prompt_toolkit.history.History` instance and pass it every time.
